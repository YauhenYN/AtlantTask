...
function DetailsPage() {
    ...
    useEffect(() => {
        async function dispatches() {
            await GetManyDetails(details, setDetails, -1, setPageNumber, setPagesCount, setIsLoaded)();
        };
        dispatches();
    }, []);
    return (<>
        ...
        {pageNumber + 1 < pagesCount && <SmallButton text="···" onClick={GetManyDetails(details, setDetails, pageNumber, setPageNumber, setPagesCount, setIsLoaded)} />}
    </>);
}

function GetManyDetails(details, setDetails, pageNumber, setPageNumber, setPagesCount, setIsLoaded) {
    return () => {
        setIsLoaded(false);
